...
public class directionalFire : MonoBehaviour {
	...
	void Update () {
		if(_shotPauseTime == 0) {
			/*
			Sprite bulletSprite = Sprite.Create (bulletTexture,
			                               ...
			                               */
			SpriteRenderer bulletRenderer = new SpriteRenderer();
			bulletRenderer.sprite = Sprite.Create (bulletTexture,
			                                       new Rect(this.transform.position.x, this.transform.position.y,
			         bulletTexture.width, bulletTexture.height),
			                                       new Vector2(bulletTexture.width / 2.0f, bulletTexture.height / 2.0f));
			...
			Instantiate(bulletRenderer, this.transform.position, q);
